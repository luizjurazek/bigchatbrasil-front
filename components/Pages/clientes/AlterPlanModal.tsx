...
export default function UpdatePlanModal({ id, initialPlan }: { id: number, initialPlan: string }) {
  ...
  const plansToDisplay = Object.values(planType).filter(plan => plan !== initialPlan);
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
            ...
            <SelectContent>
              {plansToDisplay.map((plan) => (
                <SelectItem key={plan} value={plan}>
                  {plan === planType.POS_PAGO ? "Plano Pós-Pago" : "Plano Pré-Pago"}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          <DialogFooter>
            ...
            <Button variant="default" onClick={handleUpdate}>Alterar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
